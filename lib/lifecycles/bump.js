...
function Bump (args, version) {
  ...
  return runLifecycleScript(args, 'prerelease')
    ...
    .then((stdout) => {
      console.log('--------- stdout:', stdout && stdout.trim().length && stdout.trim())
    ...
    .then((release) => {
      if (!args.firstRelease) {
        ...
        newVersion = !args.allowNonSemver ? semver.valid(releaseType) || semver.inc(version, releaseType, args.prerelease) : releaseType
    ...
    .catch(err => printError(args, err.message) && throw err)
}
...
function bumpVersion (releaseAs, currentVersion, args) {
  return new Promise((resolve, reject) => {
    console.log(releaseAs, currentVersion, JSON.stringify(args, null, 4))
    if (releaseAs) {
      return resolve({
